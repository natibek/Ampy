...
{
  /* Verifies if the file exists and has an .ampy extension. Exits with code 1 if neither is true.
   * 
   * Args: 
...
library_keywords read_library_keywords_from_bin(std::ifstream &library_bin_input) {
   ...
   * when a key can't be read.
   *
   * Args:
   *    library_bin_input: The input filestream for the binary containing the `Ampy` keywords for 
   *                      python library functions and keywords.
   *
  ...
  while (std::getline(library_bin_input, key, '\0')) {
    if (key == "-1") {
      ...
      if (!std::getline(library_bin_input, lib, '\0')) break;
      str_map inner_map;
    } 
  ...
  if (stoi(keywords_size) != read_keyword_count) {
    std::cout << "\nERROR => Read " << read_keyword_count << " | Expected " << stoi(keywords_size) << '\n';
    std::cout << "Download the correct library keyword binary.";
    exit(1);
  }
  std::cout << read_keyword_count << " pairs successfully extracted from `library_keywords.bin`.\n";
...
builtin_keywords read_builtin_keywords_from_bin(std::ifstream &builtin_bin_input) {
   ...
   *
   * Args:
   *    builtin_bin_input: The input filestream for the binary containing the `Ampy` keywords for 
   *                      python builtin functions and keywords.
   *
  ...
  if (stoi(key_words_size) != read_keyword_count) {
    std::cout << "\nERROR => Read " << read_keyword_count << " | Expected " << stoi(key_words_size) << '\n';
    std::cout << "Download the correct builtin keyword binary.";
    exit(1);
  }

  std::cout << read_keyword_count << " pairs successfully extracted from `builtin_keywords.bin`.\n";
  return builtin_map;
}

void read_keys_from_bin(builtin_keywords &builtin_map, library_keywords &library_map) {
  /* Function to help read both `builtin_keywords.bin` and `library_keywords.bin` using their
   * respective functions. Makes sure the files can be accessed and creates input filestreams for 
   * them to be used by the functions that generate the `unordered_map` for either.
   *
   * Args:
   *    builtin_map: Reference of a `builtin_keywords` `unordered_map` that will be filled with 
   *        what is read from the `builtin_keywords.bin` binary.
   *    library_map: Reference of a `library_keywords` `unordered_map` that will be filled with 
   *        what is read from the `library_keywords.bin` binary.
   */
  ...
  std::ifstream builtin_bin_input ("./../data/builtin_keywords.bin", std::ios_base::binary);
  if (!builtin_bin_input.is_open()) {
    std::cout << "Error opening with the `builtin_keywords` binary\n";
    exit(1);
  }

  std::ifstream library_bin_input ("./../data/library_keywords.bin", std::ios_base::binary);
...
int main(){
  ...
  std::cout << "ከሆነ = " << builtin_map["ከሆነ"] << " ባየሽ = " << library_map["ባየሽ"]["ባየሽ"] << " ጊዜ = " << library_map["ጊዜ"]["ጊዜ"];
}
