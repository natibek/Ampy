...
void generate_builtin_key_bin() {
  ...
  for (Json::Value::const_iterator it = myJson.begin(); it != myJson.end(); ++it) {
    bin_out.write(it.key().asString().c_str(),it.key().asString().length() + 1);
...
void generate_library_key_bin() {
  ...
  std::ifstream json_file ("./../data/library_keywords.json", std::ifstream::binary);
  Json::Value myJson;
  json_file >> myJson;
  ...
  for (Json::Value::const_iterator it = myJson.begin(); it != myJson.end(); ++it) {
    bin_out.write(it.key().asString().c_str(), it.key().asString().length() + 1);
    auto inner_map = (*it);
    for (Json::Value::const_iterator inner_it = inner_map.begin(); inner_it != inner_map.end(); ++inner_it) {
      bin_out.write(inner_it.key().asString().c_str(), inner_it.key().asString().length() + 1);
      bin_out.write((*inner_it).asString().c_str(), (*inner_it).asString().length() + 1);
    }
    bin_out.write("-1", 3);
  }
